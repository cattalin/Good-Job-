...
module.exports.searchByVideo = function(query, callback){
    Voters.find( {videoId: query.videoId}, callback );
}
...
//     return Voters.findOne( {videoId:query.videoId, voterId:query.voterId} ).select('rating').exec();
// };
...
module.exports.removeVote = function(query, callback){
    Voters.remove({videoId:query.videoId, voterId:query.voterId}, callback);
}

//updates a voter without recalculation of the rating
module.exports.updateVote = function(query, data, callback){
    Voters.update({videoId: query.videoId, voterId: query.voterId},
